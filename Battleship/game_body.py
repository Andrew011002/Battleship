...
def game(saves = []):
    ...
    while checkGrid(computerAI.grid) == checkGrid(playerOne.grid):
        ...
        time.sleep(0.5) # buffer of asking for player's & AI's choice

        # player goes first
        if turn:
            PLAYER = askPlayer()
            SPACER
            time.sleep(0.75) # time before asking AI
            if count > 0:
                # ship hit
                if hitFlag:
                    # grab ship
        ...
        # AI goes first
            ...
            # Runs as normal if not
                ...
                AI = askAI()
            SPACER
            time.sleep(1.25) # time before asking player
            PLAYER = askPlayer()
        time.sleep(1) # Time before seeing the results then seeing results (makes sense)
        # self explainatory
        last_AI_choice = (AI[1], convertAlpha(AI[0]))
        time.sleep(1) # Time before showing results of the round
        ...
        if updateGrid(playerOne.grid, AI):
            ...
            if sinkedShip(playerOne.ships):
                ...
                destroyedAI = 9
                time.sleep(1.25) # time before ship destroyed message
                display("AI: I have sucken one of your ships!", color = bcolors.RED, style = "")

            streakAI += 1
            if streakAI == 2:
                time.sleep(1.25) # time before streak message
        ...
        if updateGrid(computerAI.grid, PLAYER):
            ...
            if streakPlayer == 2:
                time.sleep(1.25) # time before streak message for player
            ...
            shipFinder((PLAYER[1], convertAlpha(PLAYER[0])), computerAI.ships) # Time before showing results of the round
            ...
            time.sleep(2) # time before showing results of the round
            ...
            streakPlayer = 0
            time.sleep(2) # time before showing results of the round comment
